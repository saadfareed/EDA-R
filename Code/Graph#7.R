...
createHistogram <- function(myData){
  ...
  myPlot <- ggplot(myData, aes(y=atm_name,x=amount_withdrawn_cub_card,color = working_day))+geom_bar(stat='identity')
  
  
  #myPlot <- myPlot + geom_bar(stat = "Count")
  myPlot <- myPlot+labs(x="Amount Withdrawn from XYZ card",y="ATM NAME")+ggtitle("Amount Withdrawn from XYZ card IN WORKING DAYS   ")
