...
createHistogram <- function(myData){
  ...
  myPlot <- ggplot(myData, aes(y=atm_name,x=amount_withdrawn_cub_card,color = weekday))+geom_bar(stat='identity')
  
  
  #myPlot <- myPlot + geom_bar(stat = "Count")
  myPlot <- myPlot+labs(x="Total Amount withdrawn From XYZ card",y="ATM NAME")+ggtitle("      Amount withdrawn by XYZ card in Weekdays   ")
