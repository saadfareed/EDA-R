...
createScatter2 <- function(myData){
  ...
  myPlot <- ggplot(myData, aes(x = total_amount_withdrawn, y = no_of_cub_card_withdrawals,,color = working_day)) +
    geom_point() +
    geom_smooth(method = 'lm', se = TRUE, level = 0.7, 
                color = 'red', fill = 'blue')+labs(x="Total Amount Withdrawn",y="No Of Withdrawals by XYZ card")+ ggtitle("    Amount of Money Withdrawls by XYZ card in Working Days  ")
