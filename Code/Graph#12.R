...
createScatter2 <- function(myData){
  ...
  myPlot <- ggplot(myData, aes(x = total_amount_withdrawn, y = No_Of_Withdrawals,,color = weekday)) +
    geom_point() +
    geom_smooth(method = 'lm', se = TRUE, level = 0.7, 
                color = 'red', fill = 'blue')+labs(x="Total Amount Withdrawn",y="No Of Withdrawals")+ ggtitle("Amount of Money in week days vs Number of Withdrawls  ")
