...
createHistogram <- function(myData){
  ...
  myPlot <- ggplot(myData, aes(y=atm_name,x=total_amount_withdrawn,color = weekday))+geom_bar(stat='identity')
  
  
  #myPlot <- myPlot + geom_bar(stat = "Count")
  myPlot <- myPlot+labs(x="Total Amount withdrawn",y="ATM NAME")+ggtitle("              Amount withdrawn in Weekdays   ")
